...
parser$add_argument("sampleFolder", help = "Path to sample folder")
...
#TargetFolder <- split.vars[6]

### Setting paths for RC, mappability, GC-content e target ###
PathGC <- file.path(args$targetFolder, "GCC")
PathMAP <- file.path(args$targetFolder, "MAP")
...
CHR <- readLines(con = TargetChrom, n = -1L, ok = T, warn = T, encoding = "unknown")
...
#### Loading RC Data ####
PathRC <- file.path(args$sampleFolder, "RC")
...
PathNorm <- file.path(args$sampleFolder, "RCNorm")
PathImages <- file.path(args$sampleFolder, "Images")
FileOut <- file.path(PathNorm, paste0(args$sampleName, ".NRC.RData"))

### Loading target, mappability and GC content Files ###
TargetOut <- loadTarget(args$targetFolder, unique.chrom)
...
errbar(
  xstep,
  MedianVecMAPNOut,
  QuantileVecMAPNOut[, 2],
  QuantileVecMAPNOut[, 1],
  axes = F,
  frame.plot = T,
  xlab = labelx,
  ylab = labely,
