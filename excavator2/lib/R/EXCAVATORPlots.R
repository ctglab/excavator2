...
ExpName <- unlist(yaml.load_file(input = ExperimentalFile))
...
for (zz in 1:length(ExpTest)) {
  ...
  FileSeg <- file.path(PathOut, paste0("HSLMResults_", ExpTest[zz], ".txt"))
  FileCall <- file.path(PathOut, paste0("FastCallResults_", ExpTest[zz], ".txt"))
  ############   ##################
