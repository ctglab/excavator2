...
loadRC <- function(PathRC, RCFiles, unique.chrom) {
  RCT <- c()
  stringSupp <- paste0(".RC.", unique.chrom, ".RData")
...
loadTarget <- function(PathTarget, unique.chrom) {
  fileTarget <- Sys.glob(file.path(PathTarget, "*.RData"))
  load(fileTarget)
  ...
  unique.chromGC <- paste0("GCC.", unique.chrom, ".RData")
  unique.chromMAP <- paste0("Map.", unique.chrom, ".RData")
  ...
  Result <- list(MyTarget, GCContentT, MAPT)
}
